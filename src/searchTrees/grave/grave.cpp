...
namespace searchTrees
{
    ...
    std::vector<float> Grave::search(common::State *state_ptr, int simulation_count, std::chrono::duration<int, std::milli> minimum_duration, float temperature)
    {
        ...
        {
            z=node.simulateOne(&node, save_illegal_amaf_actions, out_our_actions, out_their_actions, min_ref_count_, b_squared_);
        ...
        {
            z=node.simulateOne(&node, save_illegal_amaf_actions, out_their_actions, out_our_actions, min_ref_count_, b_squared_);
            out_our_actions.clear();
            out_their_actions.clear();
            i++;
        }
